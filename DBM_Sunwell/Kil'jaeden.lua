...
Kil.Version	= "0.92"
Kil.Author	= "Tandanu, Mart"
Kil.MinRevision = 1062
...
local function announceFire(self)
	...
	if phase == 2 then
		self:StartStatusBarTimer(25, "Fire Bloom CD", 45641)
		self:StartStatusBarTimer(20, "Fire Bloom", 45641)
	elseif phase == 3 then
		self:StartStatusBarTimer(30, "Fire Bloom CD", 45641)
		self:StartStatusBarTimer(20, "Fire Bloom", 45641)
	elseif phase == 4 then
		self:StartStatusBarTimer(30, "Fire Bloom CD", 45641)
		self:StartStatusBarTimer(20, "Fire Bloom", 45641)
	elseif phase == 5 then
		self:StartStatusBarTimer(45, "Fire Bloom CD", 45641)
		self:StartStatusBarTimer(20, "Fire Bloom", 45641)
	end
end
...
--		end
	elseif msg == "Phase" then
		phase = phase + 1
		if phase == 3 then
			self:StartStatusBarTimer(169, "Phase 3", "Interface\\Icons\\Spell_Shadow_UnholyFrenzy")
			self:StartStatusBarTimer(65, "Fire Bloom CD", 45641)  -- ok
			self:StartStatusBarTimer(60, "Flame Darts", 45740)
			...
			--end
			self:StartStatusBarTimer(37, "Dragon Orb", 45109) -- ok
		...
		elseif phase == 4 then
			...
			self:StartStatusBarTimer(169, "Phase 4", "Interface\\Icons\\Spell_Shadow_UnholyFrenzy")
			self:EndStatusBarTimer("Fire Bloom CD")
			self:EndStatusBarTimer("Phase 3")
			self:EndStatusBarTimer("Flame Darts")
			self:EndStatusBarTimer("Next Darkness")
			self:StartStatusBarTimer(65, "Fire Bloom CD", 45641)
			self:StartStatusBarTimer(60, "Flame Darts", 45740)
		...
		elseif phase == 5 then
			...
			self:EndStatusBarTimer("Flame Darts")
			self:StartStatusBarTimer(80, "Fire Bloom CD", 45641)
			self:StartStatusBarTimer(75, "Flame Darts", 45740)
			self:EndStatusBarTimer("Next Darkness")
			self:UnScheduleAnnounce(DBM_KIL_WARN_DARKNESS_SOON, 3)
			self:StartStatusBarTimer(59, "Next Darkness", 46605)
	...
	elseif msg == "Darts" then
		self:StartStatusBarTimer(25, "Flame Darts", 45740)
			--if self.Options.WarnDarts then
				--self:ScheduleAnnounce(34, DBM_KIL_WARN_DARTS_SOON, 1)
			--end
