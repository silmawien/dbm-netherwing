...
function Mag:OnCombatStart(delay)
	...
	self:ScheduleSelf(120, "FirstBlastNova");
end
...
function Mag:OnEvent(event, arg1)
	if event == "SPELL_CAST_SUCCESS" then
		...
		if arg1.spellId == 30576 then
			self:StartStatusBarTimer(48, "Quake", "Interface\\Icons\\Spell_Nature_Earthquake");
		end
	...
	elseif event == "CHAT_MSG_MONSTER_YELL" then
		if arg1 and string.find(arg1, DBM_MAG_YELL_PHASE2) then -- to support stupid german localization :(
			...
			self:StartStatusBarTimer(58, "Blast Nova", "Interface\\Icons\\Spell_Fire_SealOfFire");
