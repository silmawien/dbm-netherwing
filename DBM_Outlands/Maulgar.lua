...
Maulgar:RegisterCombat("YELL", {"Gronn are the real power in Outland!"}); -- stupid yells
...
function Maulgar:OnEvent(event, arg1)
	if event == "SPELL_AURA_APPLIED" then
		...
		elseif arg1.spellId == 33054 then
			if arg1.destName == "Krosh Firehand" then
				if self.Options.WarnSpellShield then
					self:Announce(DBM_MAULGAR_WARN_SHIELD, 3);
				end
				self:StartStatusBarTimer(30, "Next Spell Shield", "Interface\\Icons\\Spell_Arcane_ArcaneResilience");
			end
	...
	elseif event == "SPELL_CAST_SUCCESS" and arg1.spellName == "Arcing Smash" then
		self:SendSync("Arcing Smash");
	end
end
...
function Maulgar:OnSync(msg)
	...
	elseif msg == "Felhunter" then
		if self.Options.WarnFelhunter then
			self:Announce("New Felhunter Incoming", 2);
		end
		self:StartStatusBarTimer(48.5, "Next Felhunter", "Interface\\Icons\\Spell_Shadow_SummonFelHunter");
	elseif msg == "Arcing Smash" then 
