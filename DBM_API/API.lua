...
DBM.Version = "3.25" -- used for "a new version of dbm is available"-spam
DBMGUI_VERSION = "3.02"

DBM_REVISION = tonumber(("$Revision: 277 $"):sub(12, -3) or 0) + 784 -- 784 = revision number from the old SVN repository
DBM_VERSION = "3.263" -- this version is used for /dbm ver commands
...
function DBM.OnLoad()
	...
	SlashCmdList["LVNAXXRAMASBOSSMODS"] = function(msg)
		...
		elseif string.sub(msg, 0, 5) == "break" then
			print("yo")
			if DBM.Rank >= 1 then
				local _, _, timer = string.find(msg, "%w+ (%d+)");
				if tonumber(timer) then
					SendAddonMessage("MECBREAK", timer, "RAID");
				end
			else
				DBM.AddMsg(DRT_INVITE_NOPERMISSION);
			end
...
function DBM.OnEvent(event, ...)
	...
	elseif event == "CHAT_MSG_ADDON" and arg1 and arg2 and (arg1 == "MECPULL") then
		local timer = tonumber(arg2)
		DBM.StartStatusBarTimer(timer, "Incoming Pull");
	elseif event == "CHAT_MSG_ADDON" and arg1 and arg2 and (arg1 == "MECBREAK") then
		local timer = tonumber(arg2)
		DBM.StartStatusBarTimer(timer, "Break");
		...
		if DBM.Bosses[GetRealZoneText()] and not DBM.InCombat then
			...
			if bosses then
				for index, value in pairs(bosses) do
					if value then
						DBM.Schedule((bossTable[index].value.delay or 1), 	function(bossID)							
							if DBM.Bosses[GetRealZoneText()] and DBM.Bosses[GetRealZoneText()][bossID] then
								if DBM.DetectCombat(DBM.Bosses[GetRealZoneText()][bossID].name) then									
									DBM.CombatStart(bossID, (DBM.Bosses[GetRealZoneText()][bossID].delay or 1));
								end
							end
...
function DBM.DetectCombat(name)
	local unitID
	if UnitName("target") == name and UnitIsPlayer("target") == nil then
		unitID = "target";
	else
		for i = 1, GetNumRaidMembers() do
...
function DBM.Announce_Pull(msg, announce)
	...
	elseif msg == "pull" then
		...
		SendAddonMessage("MECPULL", time, "RAID");
