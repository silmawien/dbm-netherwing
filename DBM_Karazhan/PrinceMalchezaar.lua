...
function Prince:OnCombatStart(delay)
	Prince.Infernals 		= 0;
	Prince.Phase			= 1;
	--self:StartStatusBarTimer(31 - delay, "Infernal", "Interface\\Icons\\Spell_Shadow_SummonInfernal");
end
...
function Prince:OnEvent(event, arg1)
	...
	elseif event == "CHAT_MSG_MONSTER_YELL" then
		if arg1 == DBM_PRINCE_YELL_INF1 or arg1 == DBM_PRINCE_YELL_INF2 then
			self:ScheduleSelf(3, "InfernalSoon");
			self:ScheduleSelf(6, "InfernalSoon");
			self:ScheduleSelf(11, "Infernal");
	...
	elseif event == "InfernalSoon" then
		if self.Options.WarnInfernal then
			...
			self:AddSpecialWarning(string.format(DBM_PRINCE_WARN_INF_SOON, (self.Infernals + 1)));
			self:AddSpecialWarning(string.format(DBM_PRINCE_WARN_INF_SOON, (self.Infernals + 1)));
		end
