...
function Kael:OnSync(msg)
	...
	elseif msg == "Phase2" then
		...
		self:StartStatusBarTimer(120, "Phase 3", "Interface\\Icons\\Spell_Shadow_AnimateDead");
		self:ScheduleSelf(120, "WarnPhase3");
	...
	elseif msg == "Phase4" then
		...
		if self.Options.WarnPhase then
			...
			self:StartStatusBarTimer(11, "First MC", "Interface\\Icons\\Spell_Shadow_Charm");
		end
		...
		self:StartStatusBarTimer(35, "First Phoenix", "Interface\\Icons\\Spell_FireResistanceTotem_01");

	elseif arg1 == DBM_KAEL_YELL_PHOENIX then
		self:ScheduleSelf(60, "Next Phoenix");
		self:StartStatusBarTimer(60, "Next Phoenix", "Interface\\Icons\\Spell_FireResistanceTotem_01");

	elseif arg1 == DBM_KAEL_YELL_MINDCONTROL then
		self:ScheduleSelf(20, "Next MC");
		self:StartStatusBarTimer(20, "Next MC", "Interface\\Icons\\Spell_Shadow_Charm");
	...
	elseif msg == "Phase5" then
		...
		if self.Options.WarnPhase then
			...
			self:UnScheduleSelf("Next MC");
			self:UnScheduleSelf("Next Phoenix");
			self:EndStatusBarTimer("Next MC");
	...
	elseif msg == "Egg" and not gravityLapse then
		...
		self:StartStatusBarTimer(10, "Rebirth", "Interface\\Icons\\INV_Relics_TotemofRebirth");
...
function Kael:UpdateHealth()
	if weaponFrame then
		for i = 1, 7 do
			...
			if weaponHealth[i] > 0 then
				getglobal(weaponFrame["Bar"..i]:GetObject():GetName().."RightText"):SetText(weaponHealth[i].."HP");
			else
	...
	for i = 1, 4 do
		...
		if addHealth[i] > 0 then
			getglobal(addFrame["Bar"..i]:GetObject():GetName().."RightText"):SetText(addHealth[i].."HP");
		else
