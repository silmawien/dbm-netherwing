...
Leotheras:AddBarOption("Inner Demons in")
...
function Leotheras:OnEvent(event, arg1)
	if event == "CHAT_MSG_MONSTER_YELL" then
		if string.find(arg1, DBM_LEO_YELL_DEMON) then
			...
			self:StartStatusBarTimer(16, "Inner Demons in", "Interface\\Icons\\Spell_Shadow_ManaFeed");
		...
		elseif arg1 == DBM_LEO_YELL_SHADOW then
			...
			self:StartStatusBarTimer(16.5, "Next Whirlwind", "Interface\\Icons\\Ability_Whirlwind");
			self:ScheduleSelf(11.5, "WhirlWarn2");
...
function Leotheras:OnSync(msg)
	...
	elseif msg == "FadeWW" and self.Phase ~= "demon" then
		...
		if not self:GetStatusBarTimerTimeLeft("Demon Form") or self:GetStatusBarTimerTimeLeft("Demon Form") > 18 then
			self:StartStatusBarTimer(21, "Next Whirlwind", "Interface\\Icons\\Ability_Whirlwind");
			self:ScheduleSelf(16, "WhirlWarn");
		end
