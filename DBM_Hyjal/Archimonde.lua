...
Archimonde:AddOption("RangeCheck", true, DBM_ARCHIMONDE_OPTION_RANGECHECK)
...
function Archimonde:OnCombatEnd()
if self.Options.RangeCheck then
	...
	elseif event == "CheckTarget" then
		...
		if target then
			if self.Options.Burst then
				self:AddSpecialWarning(DBM_ARCHIMONDE_WARN_BURST:format(target), 1)
			end
...
function Archimonde:OnSync(msg)
	...
	elseif msg == "Burst" then
		self:ScheduleSelf(0.4, "CheckTarget")
	end
end
